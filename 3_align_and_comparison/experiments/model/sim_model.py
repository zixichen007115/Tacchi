...
from ..model.configs import smartlab_gelsight2014_config, mit_gelsight2017_config
...
def test():
    ...
    # print('---------_> ', cv2.CV_32F1)
    depth = np.load(PKG_PATH + "/dataset/sim/depth2/random__6__-1_-1_5.npy")
    print(np.shape(depth), np.max(depth), np.min(depth), depth.dtype)
    out2014 = smartlab_gelsight2014.generate(depth)
    out2017 = mit_gelsight2017.generate(depth)
