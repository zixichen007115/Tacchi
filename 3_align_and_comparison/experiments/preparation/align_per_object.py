...
from ..data_loader import load_single_img, load_single_img2, DataGenerator
...
def align(generator):
    ...
    def findH(img, img_sim, cls_name):
        img_pts = []
        cv2.imshow('img', img)
        ...
        cv2.imwrite('aligned/background_' + cls_name + '.png', cropped_background_img_t)
    ...
    for (real, fake, depth, cls, path) in generator:
        ...
        if cls_name == "cross_lines" or cls_name == "wave1":
            img_num = 65
        elif cls_name == "curved_surface":
            img_num = 32
        elif cls_name == "cylinder_side":
            img_num = 87
        elif cls_name == "flat_slab" or cls_name == "parallel_lines":
            img_num = 98
        else:
            img_num = 54

        # h, crop_pts = findH(np.copy(real[98]), np.copy(fake[98]), cls_name)
        h, crop_pts = findH(np.copy(real[img_num]), np.copy(fake[img_num]), cls_name)
...
if __name__ == '__main__':
    generator = DataGenerator(
        'unaligned/real',
        sim_path='unaligned/sim',
        depth_path='unaligned/depth',
        shuffle=False,
        batch_size=99,
        resize=False
    )
